...
class StreamerSourceForeachThrowsSpec extends WhiteThrowsSpec {
  ...
  it should "produce NullPointerException when convertFile(File, File) is invoked" in {
    assertThrows[NullPointerException] {
      new StreamerSourceForeach(new ProviderFake()).convertFile(null.asInstanceOf[File], null.asInstanceOf[File])
    }
  }
  it should "produce NullPointerException when convertFile(URL, URL) is invoked" in {
    assertThrows[NullPointerException] {
      new StreamerSourceForeach(new ProviderFake()).convertFile(null.asInstanceOf[URI], null.asInstanceOf[URI])
    }
  }
  it should "produce NullPointerException when convertFile(String, String) is invoked" in {
    assertThrows[NullPointerException] {
      new StreamerSourceForeach(new ProviderFake()).convertFile(null.asInstanceOf[String], null.asInstanceOf[String])
    }
  }
