...
class FutureOpt[A](opt: Option[Future[A]])(
  implicit ec: ExecutionContext) {
  def optFuture = FutureOpt.optFuture(opt)(ec)
}

object FutureOpt {
  def apply[A](opt: Option[Future[A]])(
    implicit ec: ExecutionContext) = new FutureOpt(opt)(ec)
...
object FutureOptOps {
  implicit def toPipe[A]( opt: Option[Future[A]])(
