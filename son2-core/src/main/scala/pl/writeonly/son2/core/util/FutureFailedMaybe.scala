...
object FutureFailedMaybe {
  ...
  implicit class FutureOpt[A](opt: Option[Future[A]])(
    ...
    def optFuture: Future[A] = FutureFailedMaybe.optFuture(opt)(ec)
  }

  implicit class FutureEither[A, B](value: Either[A, Future[B]])(
      implicit ec: ExecutionContext) {
    def eitherFuture: Future[B] = FutureFailedMaybe.eitherFuture(value)(ec)
  }

  implicit class FutureTry[A](value: Try[Future[A]])(
      implicit ec: ExecutionContext) {
    def tryFuture: Future[A] = FutureFailedMaybe.tryFuture(value)(ec)
  }

  implicit class FutureOr[A, B](value: Future[A] Or B)(
      implicit ec: ExecutionContext) {
    def orFuture: Future[A] = FutureFailedMaybe.orFuture(value)(ec)
  }
