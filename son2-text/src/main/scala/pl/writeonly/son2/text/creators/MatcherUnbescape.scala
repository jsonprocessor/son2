...
class MatcherUnbescape extends Matcher {
  override def translatorMatch(p: TranslateConfig): CharSequenceTranslator = p match {
    case TranslateConfig(Escapes.ESCAPE, Formats.JAVASTRING) => StringEscapeUtils.ESCAPE_JAVA
//    case TranslateConfig(Escapes.ESCAPE, Formats.ECMASCRIPT) => JavaScriptEscape.escapeJavaScriptMinimal
