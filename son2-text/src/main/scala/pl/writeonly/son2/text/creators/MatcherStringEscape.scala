...
class MatcherStringEscape extends Matcher {
  ...
  override def apply(p: TranslateConfig): (String) => String = translatorMatch(p).translate
  ...
  def translatorMatch(p: TranslateConfig): CharSequenceTranslator = p match {
    ...
    case TranslateConfig(Escapes.ESCAPE, Formats.XML11) => StringEscapeUtils.ESCAPE_XML11
    case TranslateConfig(Escapes.ESCAPE, Formats.XML10) => StringEscapeUtils.ESCAPE_XML10
    ...
    case TranslateConfig(Escapes.UNESCAPE, Formats.XML11) => StringEscapeUtils.UNESCAPE_XML
    case TranslateConfig(Escapes.UNESCAPE, Formats.XML10) => StringEscapeUtils.UNESCAPE_XML
